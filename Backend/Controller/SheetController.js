...
const handleCreateSheet = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "Sheet created successfully.", sheet: newSheet });
  } catch (error) {
...
const handleFetchAndAddQuestions = async (req, res) => {
  try {
    ...
    if (!Array.isArray(questionsData)) return res.status(400).json({ error: "Invalid API response." });
    ...
    for (const item of questionsData) {
      ...
      if (!sheet.questions.includes(existing._id)) newQuestionIds.push(existing._id);
    }

    if (newQuestionIds.length) {
      sheet.questions.push(...newQuestionIds);
      await sheet.save();
...
const handleFollowSheet = async (req, res) => {
  try {
    ...
    const [user, sheet] = await Promise.all([User.findById(userId), Sheet.findById(sheetId)]);
    if (!user || !sheet) return res.status(404).json({ error: "User or Sheet not found." });
    ...
    return res.status(200).json({ message: "Followed the sheet.", user: rest });
  } catch (error) {
...
const handleGetSheetById = async (req, res) => {
  try {
    ...
    sheet.questions.forEach((q) => {
      grouped[q.topic] = grouped[q.topic] || [];
