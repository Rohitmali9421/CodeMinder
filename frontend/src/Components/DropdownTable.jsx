        ...
        <div className="w-full space-y-4">
            {topics.map((topicData, index) => (
                <Card key={index} className="border rounded-lg overflow-hidden">
                    <div className="flex justify-between items-center bg-orange-100 px-4 py-3 cursor-pointer" onClick={() => toggleTopic(topicData.topic)}>
                                        ...
                                        return (
                                            <TableRow key={q.questionId} className="hover:bg-gray-50">
                                                ...
                                                <TableCell className="text-blue-600 text-sm font-medium">
                                                    <a href={q.url} target="_blank" rel="noopener noreferrer" className="hover:underline">{q.title}</a>
                                                </TableCell>
                                                <TableCell>
                                                    <img src={q.platform === "leetcode" ? "https://upload.wikimedia.org/wikipedia/commons/1/19/LeetCode_logo_black.png" : "https://via.placeholder.com/30"} alt={q.platform} className="w-6 h-6 rounded" />
                                                </TableCell>
                                                <TableCell className={q.difficulty === "Hard" ? "text-red-500" : q.difficulty === "Medium" ? "text-yellow-500" : "text-green-500"}>{q.difficulty}</TableCell>
                                                <TableCell>
                                                    <div className="flex flex-wrap gap-1">
                                                        {q.topicTags.slice(0, 2).map((tag, idx) => (<Badge key={idx} variant="outline">{tag}</Badge>))}
                ...
                <SheetContent side="right" className="w-screen max-w-[75vw]">
                    ...
                    <Card className="flex-grow p-3 flex flex-col gap-3 min-h-96 overflow-auto">
                        <div className="flex justify-end gap-2">
                            ...
                            <Button onClick={() => console.log("Save Note:", noteContent)}>Save</Button>
                        </div>
